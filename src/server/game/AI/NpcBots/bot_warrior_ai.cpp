    ...
    struct warrior_botAI : public bot_minion_ai
    {
        ...
        void UpdateAI(uint32 diff)
        {
                ...
                !HasAuraName(master, GREATER_BLESSING_OF_MIGHT_1) && master->IsWithinLOSInMap(me))
            {
                ...
                {
                    SetSpellCooldown(BATTLESHOUT_1, 25000); //no initial cooldown
                    return;
                }

                SetSpellCooldown(BATTLESHOUT_1, 5000); //fail
            }
        ...
        void BreakCC(uint32 diff)
        {
            if (me->HasAuraWithMechanic((1<<MECHANIC_FEAR)|(1<<MECHANIC_SAPPED)|(1<<MECHANIC_DISORIENTED)))
            {
                ...
                {
                    if (me->getLevel() >= 40)
                        doCast(me, 23691, true); //Berserker Rage Effect (434 unused)
                    return;
                }
            }
        ...
        void Attack(uint32 diff)
        {
            ...
            /*if (disarm_cd <= diff && meleedist < 5 &&
                ...
                GC_Timer <= diff)
            {
                if (opponent->getClass() == BOT_CLASS_ROGUE  ||
                    opponent->getClass() == BOT_CLASS_WARRIOR   ||
                    opponent->getClass() == BOT_CLASS_SHAMAN    ||
                    opponent->getClass() == BOT_CLASS_PALADIN)
                {
        ...
        {
            if (IsSpellReady(INTERVENE_1, diff, false) && getrage() > 100 && !IAmFree() &&
                Rand() < (IsTank() ? 80 : 30) &&
