...
void bot_ai::BotSay(char const* text, Player* target)
{
...
void bot_ai::BotWhisper(char const* text, Player* target)
{
    if (!target && master->GetTypeId() == TYPEID_PLAYER)
        target = master;
    if (!target)
...
void bot_ai::BotYell(char const* text, Player* target)
{
    if (!target && master->GetTypeId() == TYPEID_PLAYER)
        target = master;
    if (!target)
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, bool triggered, uint64 originalCaster, bool /*setCD*//* = true*/)
{
    ...
    //check cooldown
    if (!IsSpellReady(m_botSpellInfo->GetFirstRankSpell()->Id, lastdiff))
        return false;
    ...
    if (!IAmFree() && !victim->IsWithinLOSInMap(me) && IsInBotParty(victim))
    {
...
void bot_minion_ai::SetStats(bool force, bool shapeshift)
{
    ...
    switch (myclass)
    {
        ...
        case DRUID_BEAR_FORM:        ap_mod = 1.5f; spp_mod = 1.0f; armor_mod = 2.25f; crit_mod = 1.0f; haste_mod = 0.75f; dodge_mod = 1.8f; parry_mod = 0.0f;  break;
    ...
    if (!(me->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_BLOCK))
    {
        ...
        //auras are not being taken into account for creatures
        //apply them here
        value += me->GetTotalAuraModifier(SPELL_AURA_MOD_SHIELD_BLOCKVALUE);
        value *= me->GetTotalAuraMultiplier(SPELL_AURA_MOD_SHIELD_BLOCKVALUE_PCT);
...
void bot_minion_ai::_updateRations()
{
    ...
    //drink
    if (!feast_mana && me->getPowerType() == POWER_MANA && !me->IsMounted() && !me->isMoving() && CanDrink() &&
    ...
    //check
    if (feast_mana && me->getPowerType() == POWER_MANA && me->GetPower(POWER_MANA) >= me->GetMaxPower(POWER_MANA))
    {
        feast_mana = false;
        me->RemoveAurasDueToSpell(DRINK);
    }
...
void bot_pet_ai::Regenerate()
{
    ...
    if (!regenTimer_mp && me->getPowerType() == POWER_MANA && me->GetPower(POWER_MANA) < me->GetMaxPower(POWER_MANA))
    {
        regenTimer_mp = 5000;
...
void bot_ai::SetSpellCategoryCooldown(SpellInfo const* spellInfo, uint32 msCooldown)
{
    ...
    for (BotSpellMap::iterator itr = spells.begin(); itr != spells.end(); ++itr)
    {
        ...
        if (itr->second.first == spellInfo->Id)
            continue;

        info = sSpellMgr->GetSpellInfo(itr->second.first);
        if (info && info->GetCategory() == category)
            itr->second.second = msCooldown;
    }
}
...
void bot_minion_ai::RemoveItemBonuses(uint8 slot)
{
    ...
    _stats[slot][BOT_ITEM_MOD_BLOCK_VALUE] = 0;
...
void bot_ai::OnBotSpellGo(Spell const* spell)
{
    ...
    float rec = curInfo->GetRecoveryTime();
    float catrec = curInfo->CategoryRecoveryTime;

    ////let 50% of haste affect cooldowns lol
    //if (haste)
    //{
    //    if (rec)
    //        ApplyPercentModFloatVar(rec, float(haste) / 2, false);
    //    if (catrec)
    //        ApplyPercentModFloatVar(catrec, float(haste) / 2, false);
    //}

    //Set cooldown
    SetSpellCooldown(curInfo->GetFirstRankSpell()->Id, uint32(rec));
    SetSpellCategoryCooldown(curInfo->GetFirstRankSpell(), uint32(catrec));
}
