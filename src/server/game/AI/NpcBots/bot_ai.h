...
class bot_ai : public ScriptedAI
{
    ...
    protected:
        ...
        bool doCast(Unit* victim, uint32 spellId, bool triggered = false, uint64 originalCaster = 0, bool setCD = true);
        ...
        static inline uint8 GetManaPCT(Unit* hTarget) { if (!hTarget || hTarget->isDead() || hTarget->getPowerType() != POWER_MANA || hTarget->GetMaxPower(POWER_MANA) == 0) return 100; return (hTarget->GetPower(POWER_MANA)*100/(hTarget->GetMaxPower(POWER_MANA))); }
        ...
        void BotSay(char const* text, Player* target = NULL);
        void BotWhisper(char const* text, Player* target = NULL);
        void BotYell(char const* text, Player* target = NULL);
...
class bot_minion_ai : public bot_ai
{
    public:
        ...
        //uint32 GetEquips(uint8 slot) const { return _equips[slot]; }
        bool IAmFree() const;
        void SavePosition();
        void TeleportHome();
