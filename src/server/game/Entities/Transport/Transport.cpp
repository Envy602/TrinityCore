...
Transport* MapManager::LoadTransportInMap(Map* instance, uint32 goEntry, uint32 period)
{
    ...
    sLog->outInfo(LOG_FILTER_MAPS, "Creando el transporte <---");
...
void MapManager::UnLoadTransportFromMap(Transport* t)
{
    ...
    sLog->outInfo(LOG_FILTER_MAPS, "Quitando el transporte --->");
}
...
void MapManager::LoadTransportForPlayers(Player* player)
{
    ...
    for (MapManager::TransportSet::const_iterator i = tset.begin(); i != tset.end(); ++i)
    {
        ...
        sLog->outInfo(LOG_FILTER_MAPS, "Cargando el transporte <---> Aqui hasta el de TransportSet");
    }
...
void MapManager::UnLoadTransportForPlayers(Player* player)
{
    ...
    for (MapManager::TransportSet::const_iterator i = tset.begin(); i != tset.end(); ++i)
    {
        ...
        sLog->outInfo(LOG_FILTER_MAPS, "Descargando el transporte >---< Aqui desde el de TransportSet");
    }
...
Creature* Transport::AddNPCPassengerInInstance(uint32 entry, float x, float y, float z, float o, uint32 anim)
{
    ...
    if (!creature->IsPositionValid())
    {
        sLog->outError(LOG_FILTER_MAPS, "Creature (guidlow %d, entry %d) not created. Suggested coordinates isn't valid (X: %f Y: %f)", creature->GetGUIDLow(), creature->GetEntry(), creature->GetPositionX(), creature->GetPositionY());
        delete creature;
        return 0;
    }
