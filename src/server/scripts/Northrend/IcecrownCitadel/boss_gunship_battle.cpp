    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* pCreature)
        {
            ...
            if (pInstance && pInstance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
            {
                ...
                {
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "[你不是队长，无法开启战斗] ", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
                    player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, pCreature->GetGUID());
                    return true;
                }

                player->ADD_GOSSIP_ITEM(0, "向巫妖王进军！ ", 631, 1001);
        ...
        bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*sender*/, uint32 action)
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF+2)
                pCreature->MonsterSay("跳过炮艇战. ", LANG_UNIVERSAL, player->GetGUID());
    ...
    public:
        ...
        bool OnGossipHello(Player* pPlayer, Creature* pCreature)
        {
            ...
            if (pPlayer->GetItemCount(49278, false) == 0)
                pPlayer->ADD_GOSSIP_ITEM(0, "是的，为了安全起见，请给我一个火箭背包吧. ", 631, 1);
        ...
        bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*sender*/, uint32 action)
        {
            ...
            if (action == 1)
            {
                ...
                if (curItemCount >= 1)
                {
                    pCreature->MonsterWhisper("您已经领取了一个火箭背包啦! ", player->GetGUIDLow());
                    return false;
                }
                ...
                {
                    pCreature->MonsterWhisper("您的背包，没有足够空间来存放火箭背包! ", player->GetGUIDLow());
                    return false;
                }
            }
